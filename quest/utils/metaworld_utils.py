...
def get_benchmark(benchmark_name):
    benchmarks = {
        ...
        'ML45_GUIDED': ML45GuidedBenchmark,
    }
...
class ML45GuidedBenchmark(object):
    def __init__(self):
        benchmark = metaworld.ML45()
        all_classes = dict(benchmark.train_classes)
        all_classes.update(benchmark.test_classes)
        self.train_classes = {name: all_classes[name] for name in classes['ML45_GUIDED']['train']}
        self.test_classes = {name: all_classes[name] for name in classes['ML45_GUIDED']['test']}

        self.train_tasks = []
        self.test_tasks = []
        for task in benchmark.train_tasks + benchmark.test_tasks:
            if task.env_name in classes['ML45_GUIDED']['train']:
                self.train_tasks.append(task)
            else:
                self.test_tasks.append(task)

if __name__ == "__main__":
    all_wall_tasks = ['button-press-topdown-wall-v2', 'button-press-wall-v2', 'pick-place-wall-v2', 'push-wall-v2', 'reach-wall-v2']
    task_name = all_wall_tasks[-2]
    shape_meta = {
        'action_dim': 4,
        'observation': {
            'rgb': {
    ...
    env = MetaWorldWrapper(task_name, shape_meta)
    obs, info = env.reset()
    breakpoint()
